...
interface RegistrationGridProps {
  ...
  canWrite: boolean; canManageOwnership: boolean; canInactivate: boolean; canDelete: boolean;
...
export function RegistrationGrid({ records, loading, filtered, page, totalPages, canWrite, canManageOwnership, canInactivate, canDelete, onView, onEdit, onManageOwners, onInactivate, onDelete, onPageChange, onClear, onNew }: RegistrationGridProps) {
  const columns: TableColumnDefinition<RegistrationListItem>[] = useMemo(() => [
    ...
    createTableColumn({ columnId: "actions", renderHeaderCell: () => "Ações", renderCell: (record) => <div className="registration-grid__actions" onClick={(event) => event.stopPropagation()}><Tooltip content={`Visualizar matrícula ${record.number}`} relationship="label"><Button appearance="subtle" size="small" icon={<Eye20Regular />} aria-label={`Visualizar matrícula ${record.number}`} onClick={() => onView(record)} /></Tooltip><Menu positioning="below-end"><MenuTrigger disableButtonEnhancement><Button appearance="subtle" size="small" icon={<MoreHorizontal20Regular />} aria-label={`Mais ações para matrícula ${record.number}`} /></MenuTrigger><MenuPopover><MenuList><MenuItem icon={<Eye20Regular />} onClick={() => onView(record)}>Visualizar</MenuItem>{canWrite ? <MenuItem icon={<Edit20Regular />} onClick={() => onEdit(record)}>Editar</MenuItem> : null}{canManageOwnership ? <MenuItem icon={<People20Regular />} onClick={() => onManageOwners(record)}>Gerenciar proprietários</MenuItem> : null}{canInactivate ? <MenuItem icon={<PersonProhibited20Regular />} disabled={record.status === "inactive"} onClick={() => onInactivate(record)}>Inativar</MenuItem> : null}{canDelete ? <MenuItem className="registration-menu-danger" icon={<Delete20Regular />} onClick={() => onDelete(record)}>Excluir</MenuItem> : null}</MenuList></MenuPopover></Menu></div> }),
  ], [canDelete, canInactivate, canManageOwnership, canWrite, onDelete, onEdit, onInactivate, onManageOwners, onView]);
