...
interface Props { query: string; value: CarFilters; farms: CarOption[]; canCreate: boolean; onQueryChange: (value: string) => void; onChange: React.Dispatch<React.SetStateAction<CarFilters>>; onNew: () => void; }
const farmLabel = (farms: CarOption[], id: string) => farms.find((farm) => farm.id === id)?.label ?? "Todas as fazendas";

export function CarToolbar({ query, value, farms, canCreate, onQueryChange, onChange, onNew }: Props) {
  ...
  return <div className="document-toolbar car-toolbar">
    ...
    {canCreate ? <Button className="document-toolbar__new" appearance="primary" icon={<Add20Regular />} onClick={onNew}>Novo CAR</Button> : null}
  </div>;
}
