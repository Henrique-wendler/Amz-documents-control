...
type DialogState = { kind: "none" } | { kind: "inactivate"; document: DocumentListItem } | { kind: "delete"; document: DocumentListItem } | { kind: "remove-attachment"; attachment: DocumentAttachmentView };
...
export function DocumentosPage({ onNavigate }: Props) {
  ...
  const copyPath = async (attachment: DocumentAttachmentView) => { try { await documentService.logAttachmentAccess(attachment.id, "copy_reference"); await navigator.clipboard.writeText(attachment.filePath); notify("Referência copiada para a área de transferência.", "info"); } catch { notify("Não foi possível copiar a referência.", "error"); } };
  const confirmAction = async () => { if (dialog.kind === "none") return; try { if (dialog.kind === "inactivate") { await documentService.inactivate(dialog.document.id, dialog.document.version); notify("Documento inativado."); if (details?.document.id === dialog.document.id) await refreshDetails(); } else if (dialog.kind === "delete") { await documentService.delete(dialog.document.id, dialog.document.version); notify("Documento excluído."); if (details?.document.id === dialog.document.id) setDetailsOpen(false); } else { await documentService.removeAttachment(dialog.attachment.id, dialog.attachment.version); notify("Referência de arquivo removida."); await refreshDetails(); } setDialog({ kind: "none" }); await load(filters); } catch (reason) { setDialog({ kind: "none" }); notify(reason instanceof Error ? reason.message : "Não foi possível concluir a ação.", "error"); } };
  const dialogConfig = dialog.kind === "inactivate" ? { title: "Inativar documento?", message: "O documento permanecerá no histórico, com situação cadastral inativa.", confirmLabel: "Inativar", danger: false }
    ...
    : dialog.kind === "remove-attachment" ? { title: "Remover referência?", message: `A referência ${dialog.attachment.fileName} será removida logicamente. Nenhum arquivo físico será alterado.`, confirmLabel: "Remover", danger: true } : undefined;
