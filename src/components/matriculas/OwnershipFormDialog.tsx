...
export function OwnershipFormDialog({ open, registrationNumber, owners, value, currentPercentage, saving, serviceError, onClose, onSave }: OwnershipFormDialogProps) {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); setSubmitted(true); if (Object.values(errors).some(Boolean)) return; onSave({ ownerId: draft.ownerId, type: draft.type, percentage: percentValue, startDate: toPtBrDate(draft.startDate), status: draft.status }); };
