...
export function CarToolbar({ query, value, farms, onQueryChange, onChange, onNew }: Props) {
  ...
  return <div className="document-toolbar car-toolbar">
    ...
    <Field label="Situação"><Dropdown value={{ all: "Todas", active: "Ativo", pending: "Pendente", inactive: "Inativo" }[value.status]} selectedOptions={[value.status]} onOptionSelect={(_, data) => set("status", (data.optionValue ?? "all") as CarFilters["status"])}><Option value="all">Todas</Option><Option value="active">Ativo</Option><Option value="pending">Pendente</Option><Option value="inactive">Inativo</Option></Dropdown></Field>
