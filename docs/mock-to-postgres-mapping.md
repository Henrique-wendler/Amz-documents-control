...
O schema aprovado está materializado pelas migrations `001` a `016`. O acesso remoto futuro está previsto com frontend HTTPS, backend/Edge Functions para operações privilegiadas e Supabase Cloud, mantendo secrets fora do navegador. As exportações PDF e XLSX dos sete relatórios usam Edge Function, sessão/RLS/permissions e `report_log`, sem persistir o arquivo. Arquivos documentais usam Storage privado e o File Gateway para cópia Cloud → diretório interno e disponibilização local → Cloud sob demanda; permanecem pendentes CSV, implantação/homologação no servidor real, hardening final de produção e políticas operacionais.
