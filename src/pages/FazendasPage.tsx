...
export function FazendasPage({ onNavigate }: FazendasPageProps) {
  ...
  const canInactivate = hasPermission("farms.inactivate");
  const canDelete = hasPermission("farms.soft_delete");
  ...
  const saveFarm = async (draft: FarmDraft) => {
    ...
    try { if (formFarm) { await farmService.update(formFarm.id, formFarm.version, draft); notify("Fazenda atualizada com sucesso."); } else { await farmService.create(draft); notify("Fazenda cadastrada com sucesso."); } setFormOpen(false); await load(filters); }
  ...
  const confirmInactivate = async () => { if (dialog.kind !== "inactivate") return; try { const updated = await farmService.inactivate(dialog.farm.id, dialog.farm.version); setDialog({ kind: "none" }); if (details?.farm.id === updated.id) setDetails(await farmService.getDetails(updated.id)); notify("Fazenda inativada."); await load(filters); } catch (reason) { setDialog({ kind: "none" }); notify(reason instanceof Error ? reason.message : "Não foi possível inativar a fazenda.", "error"); } };
  const confirmDelete = async () => { if (dialog.kind !== "delete") return; try { const result = await farmService.delete(dialog.farm.id, dialog.farm.version); if (!result.deleted) { setDialog({ kind: "blocked", farm: dialog.farm }); return; } setDialog({ kind: "none" }); setDetailsOpen(false); notify("Fazenda excluída."); await load(filters); } catch (reason) { setDialog({ kind: "none" }); notify(reason instanceof Error ? reason.message : "Não foi possível excluir a fazenda.", "error"); } };
  ...
  return <div className="app-shell"><Sidebar activePath="/fazendas" onNavigate={onNavigate} /><div className="app-workspace"><Header title="Fazendas" subtitle="Cadastro e gestão dos imóveis rurais" refreshing={loading} onRefresh={() => void load(filters, true)} /><main className="main-content fazendas-content">
    ...
    <section className="section-card farm-search-panel" aria-label="Busca e filtros de fazendas"><FarmToolbar query={searchInput} value={filters} states={response?.states ?? []} municipalities={response?.municipalities ?? []} hasActiveFilters={hasActiveFilters} canCreate={canWrite} onQueryChange={setSearchInput} onChange={setFilters} onClear={clearFilters} onNew={openNew} /></section>
    {error ? <DashboardMessageState kind="error" title="Não foi possível carregar as fazendas" description="Tente carregar novamente a lista de imóveis rurais." onRetry={() => void load(filters)} /> : <SectionCard className="farm-results-card" title={`${response?.total ?? 0} fazenda${response?.total === 1 ? "" : "s"}`} subtitle="Selecione um imóvel para visualizar seus dados e vínculos" action={<Badge appearance="tint" color="subtle">10 por página</Badge>}><FarmGrid records={response?.records ?? []} loading={loading} filtered={hasActiveFilters} page={response?.page ?? filters.page} totalPages={response?.totalPages ?? 1} canWrite={canWrite} canInactivate={canInactivate} canDelete={canDelete} onView={(farm) => void viewFarm(farm)} onEdit={openEdit} onInactivate={(farm) => setDialog({ kind: "inactivate", farm })} onDelete={requestDelete} onPageChange={(page) => setFilters((current) => ({ ...current, page }))} onClear={clearFilters} onNew={openNew} /></SectionCard>}
  </main></div>
  <FarmDetailsDrawer record={details} open={detailsOpen} canEdit={canWrite} onClose={() => setDetailsOpen(false)} onEdit={() => { if (details) openEdit(details.farm); }} onRegistration={(id) => { setDetailsOpen(false); onNavigate(`/matriculas?open=${id}`); }} onOwner={(id) => { setDetailsOpen(false); onNavigate(`/proprietarios?open=${id}`); }} onDocument={(id) => { setDetailsOpen(false); onNavigate(`/documentos?open=${id}`); }} onCar={(id) => { setDetailsOpen(false); onNavigate(`/car?open=${id}`); }} onSeeAllOperations={() => { setDetailsOpen(false); onNavigate("/"); }} />
  <FarmFormDrawer open={formOpen} farm={formFarm} saving={saving} canInactivate={canInactivate} serviceError={formError} onClose={() => { if (!saving) setFormOpen(false); }} onSave={(draft) => void saveFarm(draft)} />
