...
import type { OwnershipDraft, RegistrationDetailsViewModel, RegistrationDraft, RegistrationFilters, RegistrationListItem, RegistrationListResponse, RegistrationLoadMode, RegistrationOwnershipView } from "../types/matricula";
...
export function MatriculasPage({ onNavigate }: MatriculasPageProps) {
  ...
  const owners = registrationService.getOwnerOptions();
  ...
  const saveRegistration = async (draft: RegistrationDraft) => { setSaving(true); setFormError(undefined); try { if (formRegistration) { await registrationService.update(formRegistration.id, draft); notify("Matrícula atualizada com sucesso."); } else { await registrationService.create(draft); notify("Matrícula cadastrada com sucesso."); } setFormOpen(false); await load(filters); } catch (reason) { setFormError(reason instanceof Error ? reason.message : "Não foi possível salvar a matrícula."); } finally { setSaving(false); } };
  const saveOwnership = async (draft: OwnershipDraft) => { if (!details) return; setSaving(true); setOwnershipError(undefined); try { if (editingOwnership) { await registrationService.updateOwnershipLink(editingOwnership.link.id, draft); notify("Vínculo atualizado com sucesso."); } else { await registrationService.createOwnershipLink(details.registration.id, draft); notify("Proprietário vinculado com sucesso."); } setOwnershipFormOpen(false); setEditingOwnership(undefined); await refreshDetails(); await load(filters); } catch (reason) { setOwnershipError(reason instanceof Error ? reason.message : "Não foi possível salvar o vínculo."); } finally { setSaving(false); } };
  const confirmInactivate = async () => { if (dialog.kind !== "inactivate") return; const updated = await registrationService.inactivate(dialog.registration.id); setDialog({ kind: "none" }); if (details?.registration.id === updated.id) await refreshDetails(updated.id); notify("Matrícula inativada."); await load(filters); };
  const requestDelete = (registration: RegistrationListItem) => { const linked = Boolean(registration.ownershipLinkCount || registration.operationCount || registration.guaranteeCount || registration.documentCount); setDialog({ kind: linked ? "blocked" : "delete", registration }); };
  const confirmDelete = async () => { if (dialog.kind !== "delete") return; const result = await registrationService.delete(dialog.registration.id); if (!result.deleted) { setDialog({ kind: "blocked", registration: dialog.registration }); return; } setDialog({ kind: "none" }); setDetailsOpen(false); notify("Matrícula excluída."); await load(filters); };
  const confirmCloseLink = async () => { if (dialog.kind !== "close-link") return; await registrationService.closeOwnershipLink(dialog.ownership.link.id); setDialog({ kind: "none" }); notify("Vínculo encerrado."); await refreshDetails(); await load(filters); };
  const confirmDeleteLink = async () => { if (dialog.kind !== "delete-link") return; await registrationService.deleteOwnershipLink(dialog.ownership.link.id); setDialog({ kind: "none" }); notify("Vínculo excluído."); await refreshDetails(); await load(filters); };
