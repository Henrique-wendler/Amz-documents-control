...
import type { CarDetailsViewModel, CarDraft, CarFilters, CarListItem, CarListResponse, CarLoadMode, CarOption } from "../types/car";
...
export function CarPage({ onNavigate }: Props) {
  ...
  const canWrite = hasPermission("car.write");
  const canInactivate = hasPermission("car.inactivate");
  const canDelete = hasPermission("car.soft_delete");
  ...
  const [farms, setFarms] = useState<CarOption[]>([]);
  const [registrations, setRegistrations] = useState<CarOption[]>([]);
  ...
  useEffect(() => { void Promise.all([carService.getFarmOptions(), carService.getRegistrationOptions()]).then(([farmOptions, registrationOptions]) => { setFarms(farmOptions); setRegistrations(registrationOptions); }).catch(() => setError(true)); }, []);
  ...
  const save = async (draft: CarDraft) => {
    ...
    try { if (formCar) { await carService.update(formCar.id, formCar.version, draft); notify("CAR atualizado com sucesso."); } else { await carService.create(draft); notify("CAR cadastrado com sucesso."); } setFormOpen(false); await load(filters); }
  ...
  const confirmAction = async () => {
    ...
    try {
      if (dialog.kind === "inactivate") { await carService.inactivate(dialog.car.id, dialog.car.version); notify("CAR inativado."); if (details?.car.id === dialog.car.id) setDetails(await carService.getDetails(dialog.car.id)); }
      else { const result = await carService.delete(dialog.car.id, dialog.car.version); if (!result.deleted) { notify("Não foi possível excluir o CAR.", "error"); return; } notify("CAR excluído."); if (details?.car.id === dialog.car.id) setDetailsOpen(false); }
      setDialog({ kind: "none" }); await load(filters);
    } catch (reason) { setDialog({ kind: "none" }); notify(reason instanceof Error ? reason.message : "Não foi possível concluir a ação.", "error"); }
  };
  const dialogConfig = dialog.kind === "inactivate" ? { title: "Inativar CAR?", message: "O cadastro permanecerá no histórico, com situação inativa.", confirmLabel: "Inativar", danger: false }
    : dialog.kind === "delete" ? { title: "Excluir CAR?", message: `O cadastro ${dialog.car.number} será removido logicamente.`, confirmLabel: "Excluir", danger: true } : undefined;
  return <div className="app-shell"><Sidebar activePath="/car" onNavigate={onNavigate} /><div className="app-workspace"><Header title="Cadastro Ambiental Rural" subtitle="Gestão dos cadastros ambientais vinculados aos imóveis rurais" refreshing={loading} onRefresh={() => void load(filters, true)} /><main className="main-content documentos-content"><CarSummary value={response?.summary} /><section className="section-card document-search-panel" aria-label="Busca e filtros do CAR"><CarToolbar query={searchInput} value={filters} farms={farms} canCreate={canWrite} onQueryChange={setSearchInput} onChange={setFilters} onNew={openNew} /></section>{error ? <DashboardMessageState kind="error" title="Não foi possível carregar os cadastros CAR" description="Tente carregar novamente os dados ambientais." onRetry={() => void load(filters)} /> : <SectionCard className="document-results-card" title={`${response?.total ?? 0} cadastro${response?.total === 1 ? "" : "s"} CAR`} subtitle="Vínculos resolvidos pela base relacional central" action={<Badge appearance="tint" color="subtle">10 por página</Badge>}><CarGrid records={response?.records ?? []} loading={loading} filtered={hasActiveFilters} page={response?.page ?? filters.page} totalPages={response?.totalPages ?? 1} canWrite={canWrite} canInactivate={canInactivate} canDelete={canDelete} onView={(item) => void view(item)} onEdit={openEdit} onInactivate={(car) => setDialog({ kind: "inactivate", car })} onDelete={(car) => setDialog({ kind: "delete", car })} onPageChange={(page) => setFilters((current) => ({ ...current, page }))} onClear={clearFilters} onNew={openNew} /></SectionCard>}</main></div>
    <CarDetailsDrawer record={details} open={detailsOpen} canEdit={canWrite} onClose={() => setDetailsOpen(false)} onEdit={() => { if (details) openEdit(details.car); }} onFarm={(id) => { setDetailsOpen(false); onNavigate(`/fazendas?open=${id}`); }} onRegistration={(id) => { setDetailsOpen(false); onNavigate(`/matriculas?open=${id}`); }} />
    <CarFormDrawer open={formOpen} car={formCar} farms={farms} registrations={registrations} saving={saving} canInactivate={canInactivate} serviceError={formError} onClose={() => { if (!saving) setFormOpen(false); }} onSave={(draft) => void save(draft)} />
