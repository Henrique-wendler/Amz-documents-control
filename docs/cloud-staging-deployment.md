...
Não use `--include-seed`: `supabase/staging/seed.sql` é deliberadamente separado de `supabase/seed.sql` e nunca roda automaticamente. As migrations `001` a `018` devem ser o único caminho para criar schema, RLS, permissions, RPCs e bucket em um banco vazio.
...
O validador é somente leitura e verifica o histórico `001–018`, 30 tabelas públicas, RLS, ausência de policy `authenticated → ALL`, permissions críticas, bucket privado, policies de Storage, RPCs sensíveis e os `SELECT` mínimos do File Gateway.
...
- Corrija schema somente com migration incremental posterior à `018`.
