...
- Os dados atuais vêm da base relacional central: `Component → Service → Selectors/ViewModels → MockStore`.
- Nenhum componente deve importar seeds diretamente.
...
- Antes de concluir alterações de código, execute `npm run build` e, quando a alteração envolver dados, `validateMockDatabase`.
...
- `src/data/mock/`: base relacional, seeds, seletores e MockStore.
